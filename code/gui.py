...
class GUI(object):
    ...
    def process_dict(self, dict, tree, parent, is_open):
        for key, value in dict.items():
            if type(value) is str:
                if ".pdf" in value:
                    tree.insert(parent, "end", text=KEY_PREFIX+key,
                                values=value, image=self.icon_pdf)
                elif ".csc" in value:
                    tree.insert(parent, "end", text=KEY_PREFIX+key,
                                values=value, image=self.icon_csc)
                else:
                    # We insert unknown files without icon and print an error message
                    tree.insert(parent, "end", text=KEY_PREFIX +
                                key, values=value)
                    print(Storyboard.ERROR_UNKNOWN_FILE_TYPE.format(value))
            else:
                child = tree.insert(parent, "end", text=key,
                                    open=is_open, tag=Storyboard.TAG_CATEGORY)
        ...
        for file_path in file_list:
            if file_name in file_path:
                ...
                # CSC files are opened via a helper function
                    ...
                    if exit_status != 0:
                        messagebox.showerror(title=Storyboard.ERROR_COOJA_DIALOG_TITLE_GUI,
                                             message=Storyboard.ERROR_CSC_FAILED_GUI)

                # Handle unknown file types
                else:
                    messagebox.showerror(title=Storyboard.ERROR_MAIN_DIALOG_TITLE_GUI,
    ...
    def start(self):
        ...
        # Prepare resources
        self.icon_csc = tk.PhotoImage(
            file=Storyboard.IOTRAIN_FIGURES_PATH + "icon_csc.gif")
        self.icon_pdf = tk.PhotoImage(
            file=Storyboard.IOTRAIN_FIGURES_PATH + "icon_pdf.gif")
        ...
        tree.tag_configure(Storyboard.TAG_CATEGORY,
                           font=("", FONT_SIZE, "bold"))
        # Color reference: https://www.rapidtables.com/web/color/gray-color.html
        style.configure("Treeview.Heading", foreground="#696969",
                        font=("", FONT_SIZE, "bold"))
        ...
        expand_button.pack(side=tk.LEFT, expand=True, fill=tk.X,
                           padx=(PADDING_X, 0), pady=(0, PADDING_Y))
        collapse_button.pack(side=tk.LEFT, expand=True,
                             fill=tk.X, pady=(0, PADDING_Y))
        cooja_button.pack(side=tk.LEFT, expand=True,
                          fill=tk.X, pady=(0, PADDING_Y))
        quit_button.pack(side=tk.LEFT, expand=True, fill=tk.X,
                         padx=(0, PADDING_X), pady=(0, PADDING_Y))
            ...
            if tree.selection():
                ...
                if file_name_list:
                    if not os.path.isdir(Storyboard.IOTRAIN_DATABASE_PATH):
                        ...
                        self.do_file_action(
                            Storyboard.IOTRAIN_DATABASE_PATH, file_name_list[0])
                        window.config(cursor="")
                        tree.config(cursor="")
        ...
        # Disable tearoff (detachable menu) feature
        actions_menu = tk.Menu(menubar, tearoff=0)
        actions_menu.add_command(
            label=Storyboard.BUTTON_START_COOJA, command=cooja_callback)
        actions_menu.add_separator()
        actions_menu.add_command(
            label=Storyboard.BUTTON_QUIT, command=window.quit)
